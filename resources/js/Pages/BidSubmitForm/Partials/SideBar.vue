...
const totalBidAmount = computed(() => {
    return props.allBids.data.reduce((sum, bid) => {
        return sum + (Number(bid.amount) || 0)
    }, 0) ?? 0
})


const userBasedBidAmount = computed(() => {
    return props.bidDetails.data?.bid.reduce((sum, bid) => {
        return sum + (Number(bid.amount) || 0)
    }, 0) ?? 0
})
...
<template>
    <div class="lg:col-span-1">
        ...
        <!-- Bid Statistics -->
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6">
            <h3 class="text-xl font-bold text-gray-800 mb-6">এই টাস্কের বিড স্ট্যাটাস</h3>
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div class="h-full bg-blue-500" :style="{ width: bidPercentage + '%' }"></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div class="h-full bg-green-500" :style="{ width: userBasedBidTotalPercentage + '%' }"></div>
                    </div>
                </div>
            ...
            <div class="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p class="text-yellow-800 text-sm">
                    <span class="font-bold">দ্রষ্টব্য:</span> আপনার বিড জমা দেওয়ার পর ২৪ ঘন্টার জন্য বাইন্ডিং থাকবে।
                    বিড জমা দেওয়ার পরে তা বাতিল করা যাবে না।
                </p>
            </div>
        </div>
